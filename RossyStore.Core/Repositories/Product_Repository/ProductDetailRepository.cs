...
namespace RossyStore.Core.Repositories.Product_Repository
{
    public class ProductDetailRepository : IProductDetail
    {
        ...
        public void Update_Add_ProductDetail_Quantity(string product_id, string color, int size, int quantity)
        {
            var productDetail_id = this.GetIDProductDetail(product_id, color, size);
            int totalProduct = 0;
            int productDetailQuantity = 0;
            ...
            Thread t1 = new Thread(() =>
            {
                client.Set($"Products/{product_id}/Details/{productDetail_id}/product_detail_quantity", productDetailQuantity + quantity);
            });

            Thread t2 = new Thread(() =>
            {
                client.Set($"Products/{ product_id}/totalProducts", totalProduct + quantity);
            });
