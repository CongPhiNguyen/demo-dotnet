...
namespace RossyStore.Core.Repositories.Order_Repository
{
    public class OrderRepository : IOrder
    {
        ...
        public bool Update_OrderStatus(string order_id, string status)
        {
            try
            {
                if (status == "Đã hủy")
                {
                    var list_product = this.Get_Order(order_id).list_cart;
                    foreach (var item in list_product)
                    {
                        new Thread(() =>
                        {
                            productDetail_repo.Update_Add_ProductDetail_Quantity(item.id, item.Product_detail_color, item.Product_detail_size, item.Quantity);
                        }).Start();
                    }
                }
                client.Set($"Orders/{order_id}/order_status", status);
                return true;
            }
            catch
            {
